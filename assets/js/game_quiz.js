...
GameQuiz.prototype.mulai_game = function() {
	...
	console.log(game.scorm_helper.getSingleData("restart"));
	if(game.scorm_helper.getSingleData("restart") == undefined){
		var ldata = game.scorm_helper.getLastGame("game_slide_"+$this.current_settings["slide"]);
	}else{
		var ldata = [];
	}
	
	if(ldata == undefined || ldata["answer"]== undefined || ldata["answer"]== null || (game.temp == 0 && ldata["answer"].length < $this.question_data.length)){
...
GameQuiz.prototype.settingPage = function($clone) {
	...
	setTimeout(function(){
		if($current_soal["image"]){
			var image = $($clone).find("#img_soal").outerHeight();
		}else{
			var image = 0;
		}
		if($current_soal["title"]){
			var ribbon = $($clone).find(".rb-wrap").outerHeight();
		}else{
			var ribbon = 0;
		}
		var button = $($clone).find(".button_wrapper").outerHeight()?$($clone).find(".button_wrapper").outerHeight():0;
		var device = $(window).outerHeight();
		var height_soal = device-(image+ribbon+button)-10;

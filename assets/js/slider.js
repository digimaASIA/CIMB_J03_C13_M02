...
Slider.prototype.addVideoPotraitEvent = function(clone,param) {
	var $this = this;
...
Slider.prototype.playVideoPotrait = function(clone,param) {
	var $this = this;
...
Slider.prototype.stopVideoPotrait = function(clone) {
	var $this = this;
		...
		if($this.listSlider[i]["click_and_show_image"]){
			...
			for (var l = 0; l < list.length; l++) {
				...
				if(list[l]["noclick"] != undefined || !list[l]["noclick"]){
					$(cButton).click(function(e){
						$(this).find(".icon_image").attr("src","assets/image/slider/"+list[parseInt($(this).attr("index"))]["image_click"]);
						game.scorm_helper.setSingleData(list[parseInt($(this).attr("index"))]["text"],true);
			...
			for (var j = 0; j < $this.listSlider[i]["button"].length; j++) {
				...
				if($this.listSlider[i]["click_and_show_image"]){
					...
					$("#popupList .button_wrapper").click(function(e){
						...
						$("#popupList").modal("hide");
						for(var k = 0; k < list_clone.length; k++){
							console.log(game.scorm_helper.getSingleData(list_clone[k]["text"]));
							if(game.scorm_helper.getSingleData(list_clone[k]["text"]) == undefined){
								show_button = false;
								break;
							}else{
								show_button = true;
						...
						if(show_button){
							$(cloneBtn).css("visibility","visible");
						}else{
							$(cloneBtn).css("visibility","hidden");
						}
						$("#popupList").find(".slider_wrapper").slick('unslick');
					});
					if(show_button){
